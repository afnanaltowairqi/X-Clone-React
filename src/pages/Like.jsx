...
function Like() {
    ...
    React.useEffect(() => {
        ...
        const likedTweets = myTweet.filter(e => myLike.includes(e.id));


    //     const deleteTweet = (id) => {
    //         axios.delete(`https://66804b9b56c2c76b495bb562.mockapi.io/api/tweet/${id}`)
    //           .then(() => {
    //             setMyTweet((e) => e.filter((item) => item.id !== id));
    //           }); 
    //       };
    // };

    const changeLike = (id) => {
        setMyLike((e) => e.filter((item) => 
            item !== id));
          };
            ...
            <div className="w-3/5 border border-gray-600 h-auto  border-t-0">             
                <div className="flex ">
                    <div class="mx-2">
                        ...
                        <p class="mb-0 w-48 text-xs text-gray-400">0 Like</p>
                    </div>
                </div>
...
<div>
    ...
    <div className="flex  ">
    <Link to="/Profile"><button className="flex-1 px-4 py-2 m-2 flex justify-center hover:bg-[#3a3a3a] ">
